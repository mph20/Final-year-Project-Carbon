...
public class Move2D_L3 : MonoBehaviour
{
    ...
    void Slide(){
         ...
         if(Input.GetKeyDown("z") && (isGrounded == true) && (isSliding == false)){
    ...
    void isPlayerDead(){
        ...
        */
        if(gameObject.GetComponent<healthandarmor>().health <= 0){
            isDead = true;}
    ...
    private void FixedUpdate() {
        ...
        if(isSliding){
                ...
                animator.Play("L3_Dash");
        }
        else if(Input.GetKey("d") || Input.GetKey("right")){
            if(isGrounded)
                animator.Play("L3_Walk");
            if(isFlipped)
            flip();
            isFlipped = false;
        }
            ...
            if(isGrounded)
            animator.Play("L3_Idle");
        }
        
        if(!isGrounded && !isSliding){
            animator.Play("L3_Jump");
        }
